...
def read_data(i, filename, max_data_size):
    if i == 0:
        ...
        # data.shape  # 32907, 11
        # data.head(5)
    # elif i == 3: # sepsis (added by annie zhou May 2nd, 2023)
    #     '''
    #        sepsis patients data from physionet challenge
    #     '''
    #     data = pd.read_csv(filename)
    #     data.drop(columns=['pat_id'], inplace=True, axis=1)
...
def binning(past_resid, alpha):
    ...
    # choose the beta that can  minimize the width of the confidence bound
    i_star = np.argmin(width)
    return beta_is[i_star]
...
This neural network is designed for regression tasks, 
given that it has a single output neuron with a ReLU (Rectified Linear Unit) activation function, 
which is commonly used for regression problems.

n summary, the code defines a feedforward neural network with three hidden layers, 
each having 100 neurons with ReLU activation functions. 
It includes dropout regularization to prevent overfitting. 
The final layer has a single neuron for regression tasks.
Depending on your specific problem and dataset, 
you would need to compile this model, specify a loss function, and fit it to your data for training.
'''

def keras_mod(seed):
    np.random.seed(seed)
    tf.random.set_seed(seed)
    # This line creates a new Sequential model and assigns the name "NeuralNet" to it. 
    # The Sequential model is a linear stack of layers, where you add layers one by one in sequence.
    model = Sequential(name='NeuralNet')
    # This line adds a Dense layer with 100 neurons and a ReLU activation function to the model. 
    # A Dense layer is a fully connected layer, meaning each neuron in this layer is connected to every neuron in the previous layer. 
    # The ReLU activation function (Rectified Linear Unit) is used to introduce non-linearity into the model.
    model.add(Dense(100, activation='relu'))
    # This line adds another Dense layer with 100 neurons and a ReLU activation function. 
    # This is a second hidden layer in the neural network.
    model.add(Dense(100, activation='relu'))
    # This line adds a Dropout layer with a dropout rate of 0.2. 
    # Dropout is a regularization technique 
    # used to prevent overfitting in neural networks. 
    # It randomly sets a fraction (in this case, 20%) of the input units to 0 at each update during training, 
    # which helps in reducing the model's reliance on specific neurons.
    model.add(Dropout(0.2))
    # This line adds a third Dense layer with 100 neurons and a ReLU activation function. 
    # This is another hidden layer in the network.
    model.add(Dense(100, activation='relu'))
...
def strided_app(a, L, S):  # Window len = L, Stride len/stepsize = S
    ...
    n = a.strides[0]  # n==the no. of bytes to get to the same position in the next row
    # tride_tricks.as_strided: Create a view into the array with the given shape and strides.
    # shape: The shape of the new array. Defaults to x.shape.
    # strides: The strides of the new array. Defaults to x.strides.
    # S * n: the no. of bytes to get to the same position in next row
    # n: the no. of bytes to get to the same position in the next column
    # Please watch this video: https://www.youtube.com/watch?v=VlkzN00P0Bc
    # make sure the stride is not overflowing the memory
