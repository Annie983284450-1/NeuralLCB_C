...
def classification_to_bandit_problem_sepsis1(contexts, labels, df,num_actions=None):
    """Normalize contexts and encode deterministic rewards."""
    # is not actions is specified, the action is the prediction class
    ...
    # Separate categorical and continuous features
    categorical_cols = ['PRG','Insurance']  # columns to exclude from normalization, ICULOS specially included
    continuous_cols = [col for col in df.columns if col not in categorical_cols]
    # contexts[continuous_cols] = (df[continuous_cols] - df[continuous_cols].mean()) / df[continuous_cols].std()
...
class SepsisData2(object):
    def __init__(self,  
        ...
        if is_window:
            ...
            # shuffle the test and train dataset to see the difference
            random.seed(42)
            random.shuffle(sepsis_train_wins)
            random.shuffle(nosepsis_train_wins)
            random.shuffle(test_septic_wins)
            random.shuffle(test_noseptic_wins)
...
class JHData(object):
    def __init__(self,       
        ...
        if group =='septic':  
            train_df = train_df[train_df['Sepsis']== 'Positive']
            test_df = test_df[test_df['Sepsis']== 'Positive']

        elif group =='nonseptic':
            train_df = train_df[train_df['Sepsis']== 'Negative']
            test_df = test_df[test_df['Sepsis']== 'Negative']



        train_df = train_df['Sepssis'].apply(lambda x: 0 if x == 'Negative' else 1)
        test_df = test_df['Sepssis'].apply(lambda x: 0 if x == 'Negative' else 1)
        train_labels = train_df['Sepssis'] 
        test_labels = test_df['Sepssis'] 
        # Assuming 'negative' is the string that indicates a negative case
        # Drop unnecessary columns
        train_df = train_df.drop(['ID', 'Sepssis'], axis=1)
        test_df = test_df.drop(['ID', 'Sepssis'], axis=1)
        # print(f'train_df:{train_df.head()}')
        # print(f'test_df:{test_df.head()}')
        # Convert the remaining columns (features) to numpy arrays
        train_contexts = train_df.to_numpy()
        ...
        # Convert the classification problem into a contextual bandit problem
        self.train_contexts, self.train_rewards = classification_to_bandit_problem_sepsis1(train_contexts, train_labels,train_df,  num_actions)
        self.test_contexts, self.test_rewards = classification_to_bandit_problem_sepsis1(test_contexts, test_labels, test_df, num_actions)
        print('////////////AAAAAAAAfter running classification_to_bandit_problem_sepsis().........../////////')
